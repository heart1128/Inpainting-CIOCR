...
class TrainerForContinuousEdgeLine:
    ...
    def save_checkpoint(self, epoch, optim, iterations, validation, edgeF1, save_name):
...
# 'lineF1': lineF1}, save_url)
    ...
    def train(self, loaded_ckpt):
        ...
        # with open('TSR.txt', 'w') as f:
        #     for pn, _ in raw_model.named_parameters():
        #         print(pn, file=f)
        ...
        for epoch in range(config.max_epochs):
            ...
            for it, items in enumerate(tqdm(loader, disable=not_show_tqdm)):
                ...
                #                          items['mask'])
                edge, loss = model(items['img'], items['edge'], items['edge'], items['mask'])
                # print(" --------TSR 参数量-------------------------------------------------------------------------------")
                # stat(model, (4, 256, 256))
                ...
                if self.iterations % 1000 == 1 and self.global_rank == 0:
                    # edge_output = edge[:4, :, :, :].squeeze(1).cpu()
                    edge_output = ((items['edge'][:4, ...] * (1 - items['mask'][:4, ...])) 
                                   + (edge[:4, ...] * items['mask'][:4, ...])).squeeze(1).cpu()
...
# edge_P, edge_R, edge_F1, line_P, line_R, line_F1 = self.val(model, test_loader)
                    edge_P, edge_R, edge_F1 = self.val(model, test_loader)
                    model.train()
...
#                     time.time() - epoch_start))
                    ...
                    self.logger.info("Epoch: %d, edge_P: %f, edge_R: %f, edge_F1: %f, "
                    "ave_F1: %f time for 1k iter: %d seconds" %
                    (epoch, edge_P, edge_R, edge_F1, average_F1,
                    time.time() - epoch_start))
...
# edge, line, _ = model(items['img'], items['edge'], items['line'], masks=items['mask'])
                edge, _ = model(items['img'], items['edge'], masks=items['mask'])
...
#                          items['erode_mask'])
                ...
                edge, loss = model(items['mask_img'], items['edge'], items['edge'], items['erode_mask'])
                ...
                if self.iterations % 2000 == 1 and self.global_rank == 0:
                    ...
                    edge_output = (items['edge'][:4, ...] + (edge[:4, ...] * (1 - items['erode_mask'][:4, ...]))).squeeze(1).cpu()
                    edge_output = torch.cat(tuple(edge_output), dim=0)
...
#                   time.time() - epoch_start))
                    ...
                    self.logger.info("Epoch: %d, edge_P: %f, edge_R: %f, edge_F1: %f, "
                    "ave_F1: %f time for 2k iter: %d seconds" %
                    (epoch, edge_P, edge_R, edge_F1, average_F1,
                    time.time() - epoch_start))
                    # supports early stopping based on the test loss, or just save always if no test set is provided
                    good_model = self.test_dataset is None or average_F1 >= bestAverageF1
...
# LineF1 = line_F1
                        self.logger.info("current best epoch is %d" % (epoch))
                        self.save_checkpoint(epoch, optimizer, self.iterations, bestAverageF1, EdgeF1,
                                             save_name='best')

                    self.save_checkpoint(epoch, optimizer, self.iterations, average_F1, edge_F1,
                                         save_name='latest')
...
# edge, line, _ = model(items['mask_img'], items['edge'], items['line'], masks=items['erode_mask'])
                edge, _ = model(items['mask_img'], items['edge'], masks=items['erode_mask'])
