...
class ZITS:
    def __init__(self, config, gpu, rank, test=False, single_img_test=False):
        ...
        with open('FTR_Model.txt', 'w') as f:
            print(self.inpaint_model, file=f)
    ...
    def sample(self, it=None):
        ...
        if self.config.SAMPLE_SIZE <= 6:
            image_per_row = 1
        images = stitch_images(
            self.postprocess((items['image']).cpu()), # 原图
            self.postprocess(items['edge_256'].cpu()),#原edge图
            self.postprocess(items['mask'].cpu()),    # mask
            self.postprocess(items['edge'].cpu()),    # 修复edge
            self.postprocess((inputs).cpu()),         # masked图
            self.postprocess((items['predicted_image']).cpu()), # 预测图像
            self.postprocess((outputs_merged).cpu()),    # 最后结果
