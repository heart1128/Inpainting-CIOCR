...
class FFC(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        if groups != 1:
            ...
            l_groups , g_groups = groups, groups

        

        # print('in_cl=', in_cl,'out_cl=', out_cl, 'in_cg=', in_cg, 'out_cg=', out_cg, 'l_groups=', l_groups, 'g_groups=', g_groups)



        module = nn.Identity if in_cl == 0 or out_cl == 0 else nn.Conv2d
        self.convl2l = module(in_cl, out_cl, kernel_size,
                              stride, padding, dilation, l_groups, bias, padding_mode=padding_type)
        module = nn.Identity if in_cl == 0 or out_cg == 0 else nn.Conv2d
        self.convl2g = module(in_cl, out_cg, kernel_size,
                              stride, padding, dilation, 1, bias, padding_mode=padding_type)
        module = nn.Identity if in_cg == 0 or out_cl == 0 else nn.Conv2d
        self.convg2l = module(in_cg, out_cl, kernel_size,
                              stride, padding, dilation, 1, bias, padding_mode=padding_type)
        module = nn.Identity if in_cg == 0 or out_cg == 0 else SpectralTransform
        self.convg2g = module(
            in_cg, out_cg, stride, 1 if g_groups == 1 else g_groups // 2, enable_lfu, **spectral_kwargs)
