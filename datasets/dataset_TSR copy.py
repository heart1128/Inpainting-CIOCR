...
class ContinuousEdgeLineDatasetMask(Dataset):
    ...
    def load_mask(self, img, index):
        ...
        if self.training is False:
            # print('len(mask_list) = ', len(self.mask_list))
            # print(index)
    ...
    def load_wireframe(self, idx, size):
        ...
        wf = pickle.load(open(line_name, 'rb')) # 读取pkl
    ...
    """  加载segmentation替代Line作为先验条件，建筑还是用line"""

    def load_segmentation(self, idx, size):
        selected_img_name = self.image_id_list[idx]
        seg_name = self.line_path + '/' + os.path.basename(selected_img_name)
        # 读取seg图
        img = cv2.imread(seg_name)
...
# seg = self.load_segmentation(idx, self.image_size)
        # load mask
        ...
        # line = self.to_tensor(seg)  # 方便，为了line不改名
...
class ContinuousEdgeLineDatasetMaskFinetune(ContinuousEdgeLineDatasetMask):
    ...
    def __getitem__(self, idx):
        ...
        meta = {'img': img, 'mask_img': mask_img, 'mask': mask, 'erode_mask': erode, 'edge': edge,
                 'name': os.path.basename(selected_img_name)}
