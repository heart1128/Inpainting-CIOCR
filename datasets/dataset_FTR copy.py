...
# line = self.load_segmentation(index, size)
# line_256 = self.load_segmentation(index, self.str_size)
...
# batch['line_256'] = self.to_tensor(line_256)
    ...
    def load_segmentation(self, idx, size):
        ...
        seg_name = os.path.basename(selected_img_name)
        if self.training is False:
            seg_name = self.eval_line_path + '/' + seg_name
        else:
            seg_name = self.train_line_path + '/' + seg_name
        # 读取seg图
